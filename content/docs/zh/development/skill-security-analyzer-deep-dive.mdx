---
title: "Skill Security Analyzer v2.0：检测 Claude Skills 中的 40+ 恶意模式"
description: "全面分析 skill-security-analyzer v2.0 这一元技能，通过 6 阶段分析对恶意代码模式实现 100% 检测率。了解该工具如何检测命令注入、YAML 注入、拼写劫持、时间炸弹、沙箱逃逸和高级编码技术，并提供基于风险的建议。"
lang: zh
...
<Cards>
  <Card title="间接执行检测" icon="AlertTriangle">
    检测通过 getattr()、__import__()、__builtins__ 访问和基于字典的混淆进行的复杂逃避
  </Card>

  <Card title="扩展的文件覆盖" icon="Files">
    扫描所有文本文件，而不仅仅是 .py——捕获隐藏在 .txt、.md、.json 和其他扩展名中的代码
  </Card>

  <Card title="高级编码分析" icon="Code">
    检测 ROT13、zlib 压缩、XOR 编码、AST 操作和多层混淆
  </Card>

  <Card title="YAML 解析" icon="FileText">
    使用实际的 YAML SafeLoader 而不是正则表达式——可靠地捕获 !!python 指令
  </Card>

  <Card title="拼写劫持检测" icon="Search">
    Levenshtein 距离分析识别恶意导入，如 "request" vs "requests"
  </Card>

  <Card title="跨文件分析" icon="GitBranch">
    跟踪文件之间的数据流以检测多阶段攻击
  </Card>
</Cards>
...
# 验证安装
python3 -c "import yaml; import Levenshtein; print('✅ 依赖项已安装')"
```
...
**危险原因：** 逃避对 `os.system`、`eval`、`exec` 的简单字符串匹配。
...
**危险原因：** YAML 文件通常受信任；不安全的加载可实现代码执行。
...
**危险原因：** 开发人员可能不会注意到拼写错误；恶意包在导入时执行。
...
# Shell 命令
os.system(f"ping {ip_address}")  # 命令注入风险
```
...
<Steps>
  ...
  <Step>
    ...
    **实现：**
    ...
    if '__proto__' in frontmatter:
        return CRITICAL_FINDING('原型污染尝试')
    ```

    **检测率：** 在 YAML 注入测试样本上 100%。
  </Step>
  ...
  <Step>
    ...
    **捕获：**
    - `request` vs `requests`（距离=1）
    - `urlib` vs `urllib`（距离=1）
    - `numppy` vs `numpy`（距离=2）
  </Step>
  ...
  <Step>
    ...
    **示例：**
    ...
    def process_data(data):
        ...
        exec(data)  # CRITICAL：未验证的用户输入到 exec
    ```
...
<Tabs items={['样本 1-4', '样本 5-8', '样本 9-11']}>
  <Tab value="样本 1-4">
    **1. 间接执行**
    ```python
...
import requester  # 有效包
```

**解决方案：**
1. 验证包在 PyPI 上存在
2. 添加到 signatures/custom.json 中的白名单
3. 在 SKILL.md 中记录为什么使用此包
...
**工作流程：**
1. skill-quality-analyzer 调用 skill-security-analyzer（步骤 3）
2. 安全发现输入到质量分数（30% 权重）
3. CRITICAL 发现 → 自动认证失败
4. 组合报告提供全面评估
...
✅ **40+ 检测模式** 涵盖命令注入、YAML 注入、凭证窃取等
✅ **100% 测试套件检测** 在 11 个恶意样本上
✅ **6 阶段分析** 包括 YAML 解析、导入分析和数据流跟踪
✅ **高级逃避检测** 用于间接执行、编码和混淆
✅ **基于风险的建议**（REJECT/REVIEW/APPROVE）
✅ **CI/CD 集成** 具有 JSON 输出和退出代码
...
- **安全最佳实践：** [Anthropic Security Guide](https://docs.claude.com/security)
